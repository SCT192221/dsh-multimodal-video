...
- **analyze_video** — 视频时序理解（本插件核心增强）。自动提取「增强帧包」一次性送视觉模型做跨帧分析，详见下文[视频理解增强](#视频理解增强增强帧包)。
...
| `analyze_video` | 视频时序分析 | `path`（必填，本地视频绝对路径）、`prompt`（可省）、`grid_frames`（默认 16）、`scene_threshold`（默认 0.3）、`max_tokens`（默认 4096） |
...
| **diff-sheet 差分图** | 运动本身。同一采样序列的相邻帧差分，亮度 = 变化幅度 | 越亮 = 该处动得越厉害；暗部 = 静止背景。直接回答「什么在动、动得多强」 |
| **scene 关键帧** | 镜头切换瞬间。场景检测提取的完整帧 | 转场时刻的完整画面，带时间戳 |

三张（组）图随引导提示一次送视觉模型，输出按固定结构组织：内容概述 → 动态过程（对象/方向/速度/节奏）→ 镜头运用与转场 → 特效演变原理（粒子/光效从哪产生、如何扩散消散）。

- **依赖 ffmpeg**（仅此工具需要，装在 PATH 即可；vision/generate_image/show_image 完全不受影响）。未安装时调用会报错并给出安装指引。
- **零额外配置**：复用视觉通道（同一模型/端点/API Key）。
- 帧提取参数可调：`grid_frames` 4–24（长视频粗看调小、细看时序调大），`scene_threshold` 0.05–0.9（越小越敏感）。
...
- `analyze_video` 的接触图/差分图为均匀采样：极端瞬态运动（单帧闪光）仍可能不在采样点上；需要精细捕捉时可调大 `grid_frames` 或配合 `scene_threshold` 使用。
