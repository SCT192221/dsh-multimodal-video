...
| **detail-anchor 锚点帧** | 每个运动区的全幅中点帧 | 定位 zoom 放大帧在整页中的位置 |
| **detail-zoom 峰值放大帧**（核心） | 运动峰值的像素级细节。自动扫描运动能量曲线定位变化最剧烈的**瞬间**（每个 = 一次特效发生），检测运动区域包围盒，裁剪出**原始分辨率**的局部画面，以约 0.2s 间隔连拍数帧 | 相邻帧时间戳差 = 演变节奏；帧间内容差异 = 特效逐步变化过程；文字/颜色/边框等像素细节以此为准 |
| **scene 关键帧** | 镜头切换瞬间。场景检测提取的完整帧 | 转场时刻的完整画面，带时间戳 |

**为什么 zoom 帧是复刻的关键**：网页动效/特效通常只持续 300ms–1s，均匀采样（16 帧覆盖 30s 视频 ≈ 每 1.9s 一帧）根本采不到演变中间态；即便定位到运动区、每段只摊几帧全幅图，间隔仍以秒计。峰值放大把采样推进到**单次特效内部**：0.2s 间隔 × 原始分辨率裁剪，模型既能数出「几帧完成演变 → 时长」，又能逐像素读清文字与颜色。请求复刻时引导提示会强制模型量化输出：时长（精确到 0.1s 量级）、缓动类型（从位移进度推断）、近似色值、布局比例。

- **依赖 ffmpeg**（仅此工具需要，装在 PATH 即可；vision/generate_image/show_image 完全不受影响）。未安装时调用会报错并给出安装指引。
- **零额外配置**：复用视觉通道（同一模型/端点/API Key）。
- 帧提取参数可调：`grid_frames` 4–24（超过 30s 的视频自动升到 24），`detail_segments` 0–8（默认 4，设 0 禁用锚点帧），`detail_peaks` 0–12（默认 5，设 0 禁用放大帧），`detail_frames` 每峰值 2–8 帧（默认 4，覆盖峰值前后约 0.6s），`scene_threshold` 0.05–0.9（越小越敏感）。整包帧数有预算上限（约 30 张），超出时自动舍弃能量较低的峰值。
...
- `analyze_video` 的运动扫描以 4fps 粒度定位运动密集区与峰值：短于 250ms 的极端瞬态（单帧闪光）可能检测不到；峰值超过 `detail_peaks` 上限或整包帧数超预算时，自动舍弃能量较低的峰值。
