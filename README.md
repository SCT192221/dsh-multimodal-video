...
逐帧调用视觉模型 = 每帧都是无上下文的静态图：模型看不到帧间「变化」，均匀采样漏运动峰值，碎片化调用无法跨帧推理；录屏场景里页面滚动还会淹没一切运动信号。`analyze_video` 先做**全局运动补偿**（块匹配 + 半像素细化估计滚动速度并扣除），再在残差信号上定位效果事件，把时间维度编码进空间图像 + **数值证据**，一次调用看懂演变：

- **滚动分离**：滚动时段与速度（px/s）精确测量，作为数值证据直接给模型——滚动不再被误认为动效
- **contact-sheet**：N 帧均匀采样拼网格（时间递增 + 时间戳水印）→ 整体结构与时间线
- **diff-sheet**：相邻采样帧差分（亮度 = 运动幅度）→ 「什么在动、动得多强」
- **detail-zoom 事件放大帧**（复刻关键）：在残差峰值（单次特效瞬间）检测运动区域，按滚动位移对齐锁定，以约 0.2s 间隔连拍**原始分辨率裁剪**帧 → 逐像素读文字颜色；事件时长由数值证据给出
- **detail-anchor 锚点帧** + **scene 关键帧**（排除滚动误检）

所有图随结构化引导提示一次送视觉模型；复刻请求时强制量化输出时长（用证据数据）/缓动/色值/布局。用法见 `dsh-multimodal-video/README.md` 的「视频理解增强」章节。
